...
function SelectGridStyle(numRows, numCols, selection) {
  ...
  const generateVerticalLines = () => {
    ...
    const wallsToRemove = Math.floor(0.1 * totalWalls);
  ...
  const generateRandomDots = () => {
    const grid = createGrid();
    const probability = 0.6; // 5% chance for a dot
  ...
  const maze = generateMaze(selection);
